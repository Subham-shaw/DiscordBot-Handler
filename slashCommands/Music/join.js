...
module.exports = {
    ...
    run: async (client, interaction, args, prefix) => {
        try {
            ...
            if(oldConnection) return interaction.reply({ ephemeral: true, content:`<:M_x:950441844544794654> **I'm already connected in <#${oldConnection.joinConfig.channelId}>**!`}).catch(() => null);
            
            if(!interaction.member.voice.channel?.permissionsFor(interaction.guild?.me)?.has(Permissions.FLAGS.CONNECT)) {
                return interaction.reply({ephemeral: true, content: "<:M_x:950441844544794654> **I'm missing the Permission to Connect to your Voice-Channel!**"}).catch(() => null);
            }
            if(!interaction.member.voice.channel?.permissionsFor(interaction.guild?.me)?.has(Permissions.FLAGS.SPEAK)) {
                return interaction.reply({ephemeral: true, content: "<:M_x:950441844544794654> **I'm missing the Permission to Speak in your Voice-Channel!**"}).catch(() => null);
            }
