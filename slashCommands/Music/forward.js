...
module.exports = {
    ...
    run: async (client, interaction, args, prefix) => {
        try {
            ...
            if(!args[0] || isNaN(args[0])) return interaction.reply({ ephemeral: true, content:`<:M_x:950441844544794654> **You forgot to add the forwarding-time!** Usage: \`${prefix}forward <Time-In-S>\``}).catch(() => null);
            
            if(Number(args[0]) < 0 || Number(args[0]) > Math.floor((queue.tracks[0].duration - curPos) / 1000 - 1))
            return interaction.reply({ ephemeral: true, content:`<:M_x:950441844544794654> **The Forward-Number-Pos must be between \`0\` and \`${Math.floor((queue.tracks[0].duration - curPos) / 1000 - 1)}\`!**`}).catch(() => null);
