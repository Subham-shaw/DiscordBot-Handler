...
module.exports = {
    ...
    run: async (client, interaction, args, prefix) => {
        try {
            ...
            if(!args[0] || isNaN(args[0])) return interaction.reply({ ephemeral: true, content:`<:M_x:950441844544794654> **You forgot to add the rewinding-time!** Usage: \`${prefix}rewind <Time-In-S>\``}).catch(() => null);
            if(Math.floor(curPos / 1000 - 1) <= 0) return interaction.reply({ ephemeral: true, content:`<:M_x:950441844544794654> **There is nothing to rewind, play a song long enough!**`}).catch(() => null);
            if(Number(args[0]) < 0 || Number(args[0]) > Math.floor((curPos) / 1000 - 1))
            return interaction.reply({ ephemeral: true, content:`<:M_x:950441844544794654> **The Rewind-Number-Pos must be between \`0\` and \`${Math.floor((curPos) / 1000 - 1)}\`!**`}).catch(() => null);
