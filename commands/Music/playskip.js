...
module.exports = {
    ...
    run: async (client, message, args, prefix) => {
        ...
        if(!message.member.voice.channel?.permissionsFor(message.guild?.me)?.has(Permissions.FLAGS.CONNECT)) {
            return message.reply({ content: "<:M_x:950441844544794654> **I'm missing the Permission to Connect to your Voice-Channel!**"}).catch(() => null);
        }
        if(!message.member.voice.channel?.permissionsFor(message.guild?.me)?.has(Permissions.FLAGS.SPEAK)) {
            return message.reply({ content: "<:M_x:950441844544794654> **I'm missing the Permission to Speak in your Voice-Channel!**"}).catch(() => null);
        }
        ...
        if(!args[0]) return message.reply(`<:M_x:950441844544794654> Please add the wished Music via: \`${prefix}playskip <Name/Link>\``).catch(() => null);
